...
def mc_entropy(model, img, T=10):
    ...
    with torch.no_grad():
        for _ in range(T):
            prob=torch.softmax(model(img), 1)
            if sum_prob is None:
                sum_prob = prob
            else:
                sum_prob += prob
    mean_prob = sum_prob / T
...
# ---------- 3.主动学习 ----------
def active_learning_loop(model, train_dataset, val_dataloder, device,transforms,
    ...
    for r in range(rounds):
        ...
        # 训练
        model = train_epochs(model, subset_loader, val_dataloder, device, epochs_per,None)
        # 挑新样本
